class SyntaxConverter {
  static String calculatorToLatex(String calculatorSyntax) {
    ...
    latex = latex.replaceAllMapped(
      RegExp(r'sum\(([^,]+),([^,]+),([^)]+)\)'),
      (match) =>
          '\\sum_{${match.group(1)}}^{${match.group(2)}} ${match.group(3)}',
    );

    latex = latex.replaceAllMapped(
      RegExp(r'prod\(([^,]+),([^,]+),([^)]+)\)'),
      (match) =>
          '\\prod_{${match.group(1)}}^{${match.group(2)}} ${match.group(3)}',
    );
    ...
    latex = latex.replaceAllMapped(
      RegExp(r'sum\(([a-zA-Z]+)=([^,]+),([^,]+),([^)]+)\)'),
      (match) =>
          '\\sum_{${match.group(1)}=${match.group(2)}}^{${match.group(3)}} ${match.group(4)}',
    );
    ...
    latex = latex.replaceAllMapped(
      ...
      (match) =>
          '\\prod_{${match.group(1)}=${match.group(2)}}^{${match.group(3)}} ${match.group(4)}',
    );
    ...
    latex = latex.replaceAllMapped(
      ...
      (match) {
        ...
        if (numerator.contains('\\frac') ||
            denominator.contains('\\frac') ||
            numerator.contains('\\') ||
    ...
    String normalizeInfinity(String s) =>
        s.replaceAll('inf', '\\infty').replaceAll('∞', '\\infty');
    ...
    latex = latex.replaceAllMapped(
      RegExp(r'partial\(([^,]+),([^)]+)\)'),
      (m) =>
          '\\frac{\\partial}{\\partial ${m.group(2)!.trim()}} ${m.group(1)!.trim()}',
    );
    ...
    latex = latex.replaceAllMapped(
      RegExp(r'(?:limit|lim)\(([^,]+),([^,]+),([^)]+)\)'),
      (m) =>
          '\\lim_{${m.group(1)!.trim()} \\to ${normalizeInfinity(m.group(2)!.trim())}} ${m.group(3)!.trim()}',
    );
    ...
    latex = latex.replaceAllMapped(
      ...
      (m) =>
          '\\sum_{${m.group(1)!.trim()}=${m.group(2)!.trim()}}^{${m.group(3)!.trim()}} ${m.group(4)!.trim()}',
    );
    latex = latex.replaceAllMapped(
    ...
    latex = latex.replaceAllMapped(
      ...
      (m) =>
          '\\prod_{${m.group(1)!.trim()}=${m.group(2)!.trim()}}^{${m.group(3)!.trim()}} ${m.group(4)!.trim()}',
    );
    latex = latex.replaceAllMapped(
  ...
  static String _casesInnerToLatex(String inner) {
    ...
    for (final g in items) {
      ...
      if (pair.length >= 2) {
        ...
        final cond =
            (condRaw.toLowerCase() == 'otherwise' ||
                condRaw.toLowerCase() == 'else')
            ? '\\text{otherwise}'
            : '\\text{if } $condRaw';
        lines.add('$expr & $cond');
      }
  ...
  static String _matrixInnerToLatex(String inner) {
    final rows = _splitTopLevelGroups(
      inner,
    ).map((g) => _stripParens(g)).toList();
    if (rows.isEmpty) return inner;
    final rowLatex = rows
        .map((r) => _splitTopLevelCommas(r).map((e) => e.trim()).join(' & '))
        .join(' \\ ');
  ...
  static String _processSumInFraction(String expression) {
    ...
    result = result.replaceAllMapped(
      RegExp(r'sum\(([a-zA-Z]+)=([^,]+),([^,]+),([^)]+)\)'),
      (match) =>
          '\\displaystyle\\sum_{${match.group(1)}=${match.group(2)}}^{${match.group(3)}} ${match.group(4)}',
    );
    ...
    result = result.replaceAllMapped(
      RegExp(r'sum\(([^,]+),([^,]+),([^)]+)\)'),
      (match) =>
          '\\displaystyle\\sum_{${match.group(1)}}^{${match.group(2)}} ${match.group(3)}',
    );
